...
the task line). Use the #tool:vscode/askQuestions:
...
- **Finish it now** -- the task needs real implementation work. Ask a
  follow-up via **vscode/askQuestions**:
...
`openspec/changes/${input}/followups.md` to check whether the file exists.
...
- In **Manual**, use the #tool:vscode/askQuestions to ask:
