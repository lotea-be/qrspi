...
b-ter. **Write the QRSPI version marker.** After the config sentinel is in
   ...
   `.github-plugin/plugin.json` `version` field (e.g. `0.5.0`) -- no `v`
   ...
   Read `.github-plugin/plugin.json` to obtain the version string. The content
...
`.github/instructions/*-stack.instructions.md`) whether this repo already has one — do not
shell out.

**No stack-cheatsheet skill yet (mandatory ask):** use the #tool:vscode/askQuestions to ask whether to bootstrap it now:
