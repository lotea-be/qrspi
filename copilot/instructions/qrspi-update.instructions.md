...
walk gates judgment steps with `vscode/askQuestions`, a main-loop-only tool.
...
version/manifest — read `.github-plugin/plugin.json` `version` from the
...
> `~/.copilot/plugins/cache/<marketplace>/qrspi/<version>/`) and multiple
...
2. Offer, via **`vscode/askQuestions`**, to either:
...
This is a real `vscode/askQuestions` gate and therefore MUST run on the main loop
...
do not stop for a separate confirmation here unless a step's `vscode/askQuestions`
...
1. **Present the step** via `vscode/askQuestions`:
