...
(AskUserQuestion), **downgrade** (one-line warning), and **unreadable-B / config
...
- **If `openspec/` exists but `openspec/.qrspi-version` does not:** hand off
  ...
  competing AskUserQuestion from this skill -- `/qrspi:update` owns that
  gate. Re-enter `/qrspi:update` on the main loop as a slash command (not
  as a subagent spawn) so its AskUserQuestion gate fires normally.
...
subagent spawn -- a subagent cannot issue AskUserQuestion, which the update
...
Do NOT issue an AskUserQuestion. Set the in-context session flag and return.
