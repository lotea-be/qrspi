---
description: Update a QRSPI-initialized repo to a newer kit version. Reads the openspec/.qrspi-version marker, walks each intervening release's migration entry in ascending SemVer order, hybrid-applies mechanical steps (edit-file, no prompt), gates judgment steps via AskUserQuestion, then bumps the marker and prints a ready-to-run git commit command. Runs on the main loop so AskUserQuestion gates are reachable.
---
...
5. Runs the apply phase per the skill's "Apply phase" section:
   ...
   - **Manual steps**: surfaced via `AskUserQuestion`; the walk holds until the
