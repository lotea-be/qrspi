...
> New capability introduced by the `tighten-stage-read-boundaries` change.
> Defines the per-stage read-matrix contract and cross-change boundary for the
> seven QRSPI stage agents (R, Q, D, S, V, P, I, PR), plus the per-agent
> read-contract banner format and the workflow-skill table that serves as the
