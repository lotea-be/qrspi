...
> New capability introduced by the `archive-requires-merged-pr` change. The
> PR-merge gate `/qrspi:archive` runs before delegating to the generated
> `openspec-archive-change` skill, and the atomic backlog-row-removal commit
> step it runs after the skill succeeds.
