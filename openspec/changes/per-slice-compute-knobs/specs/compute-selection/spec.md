...
> New capability introduced by the `per-slice-compute-knobs` change. Unified
> `**Compute:**` annotation grammar for per-slice model selection and per-stage
> effort configuration, replacing the former `**Model:**` annotation.
