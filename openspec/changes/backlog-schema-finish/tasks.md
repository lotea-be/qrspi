...
- [ ] 5.1 Migrate `claude/commands/design.md` step 4 ("Capture deferred work") to load `backlog-writer` and delegate row construction, removing the inline `### <slug> — \`idea\` · **P<n>**` + `**Why:**`/`**Shape:**` block while preserving the offer/dedup/skip-if-nothing prose (D11)
- [ ] 5.2 Migrate `claude/commands/structure.md`'s capture step the same way — load `backlog-writer`, remove the inline grammar block, preserve surrounding capture semantics (D11)
- [ ] 5.3 Migrate `claude/commands/pr.md`'s "Promote to backlog idea" path to load `backlog-writer` and delegate row construction, removing the inline grammar block while preserving the surrounding `followups.md` tick + commit orchestration (D11)
- [ ] 5.4 Trim the referential grammar block in `claude/commands/slices.md` to a pointer to the frozen grammar (template + Check 22) — stage V does not append, so no delegation call is needed, only removal of the full inline copy (D11)
- [ ] 5.5 Wire any command→skill registration Check 2 requires in `scripts/skill-sets.mjs` for the migrated commands (D11)
...
- [ ] 5.7 Checkpoint (automated): `node scripts/lint.mjs` exits 0; Check 2 reports `OK` for the migrated commands against `backlog-writer`; no inline row-grammar block remains outside the skill/template
