# Spec — git-host-workflow

> New capability introduced by the `git-host-and-remote-awareness` change.
> The shared how-to-derive procedure skill for git-host vendor resolution,
> remote-presence checking, branch-name resolution, and the no-remote local
> flow, loaded by `pr.md` and `archive.md`.
