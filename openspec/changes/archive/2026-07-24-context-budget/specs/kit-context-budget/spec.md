# Spec — kit-context-budget

> New capability introduced by the `context-budget` change. Provides a static,
> deterministic per-stage context-footprint estimator (`scripts/context-footprint.mjs`)
> that reports byte/line/rough-token estimates for each QRSPI stage agent and its
> declared skill set.
