...
- [x] 3.10 (human) Run `node scripts/lint.mjs` — confirm Check 8 reports OK and exit code is 0
- [x] 3.11 (human) Temporarily delete the tasks-pass section from `claude/commands/pr.md` and re-run `node scripts/lint.mjs` — confirm Check 8 reports "tasks pass missing from pr.md" and exits non-zero; restore the section afterwards
- [x] 3.12 (human) Run `node sync-copilot.mjs --check` — confirm zero drift; inspect `claude/agents/reviewer.md` for the Leave-for-now tolerance note; inspect `claude/skills/workflow/SKILL.md` for the one-line reconciliation-gate cross-reference
