...
- [x] 2.4 (human) In a `claude --plugin-dir /workspaces/git/qrspi` session on this kit, run `/qrspi:questions` on a throwaway change — confirm the produced `questions.md` contains no `## Data model`, `## API`, `## Migrations & data`, `## UI`, `## Auth & authorization`, `## Front-end state`, or `## Indexing & query performance` headings, and no "Not applicable" text under any surface-gated heading.
- [x] 2.5 (human) Build a throwaway web-app fixture outside this repo (in the scratchpad), run `/qrspi:questions` in a `--plugin-dir` session — confirm CRUD sections appear in the output (prose inference fires).
...
- [x] 3.9 (human) In a `claude --plugin-dir /workspaces/git/qrspi` session, run `/qrspi:design`, `/qrspi:structure`, `/qrspi:plan`, and `/qrspi:pr` (or a subset) on a throwaway change in this kit — confirm each produced artifact contains no CRUD headings and no "Not applicable" text under surface-gated sections.
