...
> New capability introduced by the `pr-review-open-tasks-and-followups` change.
> Pre-PR reconciliation gate: walks un-ticked tasks and un-resolved follow-ups
> before the reviewer subagent runs, ensuring every open item has an explicit
> human decision before the PR is drafted.
