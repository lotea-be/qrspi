...
> New capability introduced by the `per-slice-compute-tier` change. Three
> effort-variant implementer agents (`implementer-low/medium/high.md`) and
> the shared `implementer-core` skill that backs all four implementer files.
