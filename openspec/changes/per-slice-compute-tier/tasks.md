...
- [ ] 4.1 Edit `claude/commands/implement.md` (main spawn site): add logic to parse the next un-ticked slice's `**Compute:**` line — extract `effort=` (required; hard-stop if absent) and `model=` (optional; default `sonnet`) — then map `effort=` to the correct variant agent (`low` → `implementer-low`, `medium` → `implementer-medium`, `high` → `implementer-high`) and spawn with the resolved `model:` parameter. (D7)
- [ ] 4.2 Edit `claude/commands/implement.md` (auto-mode loop site): apply the same `effort=` → variant + `model=` resolution in the Full/Semi-auto per-slice loop so auto-chained slices also dispatch to the correct variant. (D7)
- [ ] 4.3 Wire the hard-stop: when `effort=` is absent from a slice's `**Compute:**` line, surface the condition and halt — no implementer is spawned. (D7)
- [ ] 4.4 Update Check 13 in `scripts/lint.mjs` to enforce `effort=` as required and accept `model=` as optional in the orthogonal grammar; also update the `vertical-slice` skill grammar section and `openspec-templates/` grammar comments to reflect the new canonical form. (D8)
- [ ] 4.5 Add the migration entry to `migrations/` for this version covering the grammar change (existing `slices.md` and `tasks.md` files using the old `model=`-required form). (D8)
- [ ] 4.6 Unit/integration test: `node scripts/lint.mjs` exits 0 with the updated Check 13; verify that a fixture with `model=sonnet` and no `effort=` token causes a non-zero exit. (D7, D8)
...
- [ ] 4.8 Checkpoint: `node scripts/lint.mjs` exits 0.
