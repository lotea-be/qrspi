...
- [ ] **[Added post-hoc — found at stage-I dogfood, 2026-07-27]** Agent *registration* was never mapped: `.claude-plugin/plugin.json` declares agents via an **explicit `agents` array** (not directory discovery), and spawnable subagent types are plugin-namespaced (`qrspi:<stem>`). A new `claude/agents/*.md` file is inert — unspawnable — until it is listed in that array and invoked as `qrspi:<stem>`. Stage R mapped the lint / `main()` / `SKILL_SET_EXPECTED` registry surface but not the plugin manifest, which is what let the variant mechanism ship non-functional (design D9). Retro item.
