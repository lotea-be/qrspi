...
> New capability introduced by the `versioned-update-command` change. Provides
> the version marker written into consuming repos, the migration manifest schema,
> and the `/qrspi:update` walk command + skill that drives hybrid migration
> application across intervening releases.
