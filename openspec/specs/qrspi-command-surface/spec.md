...
`claude/commands/opsx/sync.md`). These files SHALL be deleted from the repo.
...
The `README.md` MUST include `qrspi-version-check` in the skills list so that
the skill is discoverable to users and contributors. This entry MUST be added
in the same change as the skill itself, not deferred to a later PR.
