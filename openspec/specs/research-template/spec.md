# research-template Specification

## Purpose
The kit-shipped, spine-only `openspec-templates/research.template.md` and its
lint wiring. The template carries the five canonical spine headings for a QRSPI
stage-R `research.md`; surface-driven inventory headings are injected dynamically
from the repo's declared surfaces and never appear in the template. Check 3
guards the spine headings against the researcher agent's inline skeleton.
