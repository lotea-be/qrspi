# spec-syncer Specification

## Purpose
A kit-owned least-privilege helper agent that owns the authoritative delta-merge
contract and enforces the scenario-count-drop hard-stop when syncing change specs
into base specs at archive time. Created by the `spec-sync-contract` change.
## Requirements
