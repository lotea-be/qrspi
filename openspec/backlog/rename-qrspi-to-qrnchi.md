...
**Excluded from edits:** `copilot/**` (regenerated by `sync-copilot.mjs`, never
hand-edited) and `openspec/changes/archive/**` (immutable history — leave the old
`qrspi` references as record).
...
## 5. Source file inventory (source only; excludes generated `copilot/` + archive)
...
- **Dev tooling** `.claude/skills/qrspi-{dogfood,release,sync-copilot}/` and
...
- **Generation + lint** — `sync-copilot.mjs` (the `/qrspi:` → `/qrspi-` rewrite
  regex → `/qrnchi:` → `/qrnchi-`; the `qrspi-` prompt-filename prefix; any
  `agentFor`/`hintFor` tables keyed on design/structure/slices → nail/chart/hew),
  then `node sync-copilot.mjs` to rebuild `copilot/**`; `scripts/lint.mjs` (Check
...
- **Docs** — `README.md` (stage table, command list, install cmd
  `/plugin install qrnchi@lotea-agents`, two-tool mapping, acronym section, repo
  layout, + crouton vocabulary + tagline); `CONTRIBUTING.md`; `CLAUDE.md`;
  `CHANGELOG.md` (`[Unreleased]`, no version bump); `install.sh`/`install.ps1`.
...
- **Verification:**
  ...
  - `node sync-copilot.mjs --check` — zero drift.
